...
public class StoreDirect extends Store {
    ...
    public long preallocate() {
        ...
        try{
            ...
            final Lock lock  = locks[lockPos(ioRecid)].writeLock();
            lock.lock();
            try{
    ...
    protected void put2(DataOutput2 out, long ioRecid, long[] indexVals) {
        assert(locks[lockPos(ioRecid)].writeLock().isHeldByCurrentThread());
    ...
    public <A> A get(long recid, Serializer<A> serializer) {
        ...
        final Lock lock  = locks[lockPos(ioRecid)].readLock();
        lock.lock();
        try{
    ...
    protected <A> A get2(long ioRecid, Serializer<A> serializer) throws IOException {
        assert(locks[lockPos(ioRecid)].getWriteHoldCount()==0||
                locks[lockPos(ioRecid)].writeLock().isHeldByCurrentThread());
